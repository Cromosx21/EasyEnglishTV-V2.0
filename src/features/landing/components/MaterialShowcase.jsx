...
export default function MaterialShowcase() {
  ...
  return (
    <section className="py-24 bg-gradient-to-b from-brand-light to-white relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 overflow-hidden" ref={containerRef}>
        ...
        >
          {materials.map((currentMaterial, index) => (
            <div key={currentMaterial.id} className="min-w-full px-4 lg:px-12 box-border">
                <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24 relative">
                  ...
                  <div className={`hidden lg:block absolute left-0 top-1/2 -translate-y-1/2 -translate-x-12 z-20 ${index !== activeIndex ? 'pointer-events-none opacity-0' : ''}`}>
                      <button 
                        onClick={prevMaterial}
                        disabled={activeIndex === 0}
                        className={`w-12 h-12 flex items-center justify-center rounded-full bg-white shadow-lg border border-gray-100 text-brand-dark hover:bg-brand-light hover:scale-110 transition-all ${activeIndex === 0 ? 'opacity-50 cursor-not-allowed' : ''}`}
                  ...
                  <div className={`hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 translate-x-12 z-20 ${index !== activeIndex ? 'pointer-events-none opacity-0' : ''}`}>
                      <button 
                        onClick={nextMaterial}
                        disabled={activeIndex === materials.length - 1}
                        className={`w-12 h-12 flex items-center justify-center rounded-full bg-white shadow-lg border border-gray-100 text-brand-dark hover:bg-brand-light hover:scale-110 transition-all ${activeIndex === materials.length - 1 ? 'opacity-50 cursor-not-allowed' : ''}`}
                  ...
                  <div className="flex-1 space-y-8 w-full">
                    ...
                    <div className="flex justify-between items-center lg:hidden mb-4 px-4">
                       <button 
                           onClick={(e) => { e.stopPropagation(); prevMaterial(); }} 
                           disabled={activeIndex === 0}
                           className={`p-2 rounded-full bg-gray-100 hover:bg-gray-200 ${activeIndex === 0 ? 'opacity-50' : ''}`}
                        >
                           <ArrowLeft className="w-5 h-5" />
                        </button>
                       <span className="font-mono text-sm text-gray-400">{index + 1} / {materials.length}</span>
                       <button 
                           onClick={(e) => { e.stopPropagation(); nextMaterial(); }} 
                           disabled={activeIndex === materials.length - 1}
                           className={`p-2 rounded-full bg-gray-100 hover:bg-gray-200 ${activeIndex === materials.length - 1 ? 'opacity-50' : ''}`}
                        >
        ...
        <div className="flex justify-center gap-2 mt-8">
            {materials.map((_, idx) => (
                <button
                    ...
                    className={`h-2 rounded-full transition-all duration-300 ${idx === activeIndex ? 'w-8 bg-brand-blue' : 'w-2 bg-gray-300'}`}
