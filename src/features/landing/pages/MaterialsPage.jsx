...
export default function MaterialsPage() {
	...
	const filteredMaterials = materialsData
		.filter((material) => {
			...
			const matchesCategory =
				...
				selectedCategories.includes(material.level);
	 ...
	 * Agrega al carrito independientemente de si el usuario está logueado o no.
	 */
	const handleAddToCart = (e, material) => {
		e.preventDefault(); // Evitar navegación del Link padre
		e.stopPropagation(); // Detener propagación del evento

		addToCart({ ...material, type: "material" });
		// Ya no redirigimos al login aquí, permitimos carrito invitado
	};
			...
			<section className="bg-brand-yellow pt-32 py-12 relative overflow-hidden">
				...
				<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
					...
					<p className="text-brand-dark/70 font-medium">
						Descarga nuestros materiales exclusivos y aprende a tu
						ritmo.
					</p>
				</div>
			</section>
							...
							<div>
								...
								<div className="space-y-2">
									{[
										...
										<label
											key={cat}
											className="flex items-center gap-2 text-sm text-gray-600 cursor-pointer hover:text-brand-blue"
										>
											<input
												type="checkbox"
				...
				<div className="flex-1">
					...
					<div className="flex justify-between items-center mb-6">
						<p className="text-gray-500 text-sm">
							Mostrando {filteredMaterials.length} resultados
						</p>
					...
					<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
						{filteredMaterials.map((material) => (
							...
							>
								...
								<div
									className={`aspect-[3/4] rounded-xl bg-${material.image}-100 mb-4 relative overflow-hidden group-hover:scale-[1.02] transition-transform`}
								>
									...
									<div className="mt-auto flex items-end justify-between">
										<div>
											<p className="text-xs text-gray-400 line-through">
												S/{" "}
												{material.originalPrice.toFixed(
													2
												)}
											</p>
